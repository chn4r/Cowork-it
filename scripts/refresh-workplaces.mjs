...
const query=`[out:json][timeout:45];(nwr[office=coworking](around:50000,${ORIGIN.lat},${ORIGIN.lng});nwr[amenity=coworking](around:50000,${ORIGIN.lat},${ORIGIN.lng});nwr[amenity=library](around:50000,${ORIGIN.lat},${ORIGIN.lng});nwr[amenity=community_centre](around:50000,${ORIGIN.lat},${ORIGIN.lng});nwr[amenity=cafe](around:50000,${ORIGIN.lat},${ORIGIN.lng});nwr[craft](around:50000,${ORIGIN.lat},${ORIGIN.lng}););out center tags;`;
let osm=[];
try{
 ...
 osm=(j.elements||[]).filter(e=>e.tags?.name).map(e=>{
   ...
   const cowork=t.office==='coworking'||t.amenity==='coworking',lib=t.amenity==='library',community=t.amenity==='community_centre',cafe=t.amenity==='cafe';
   if(!(cowork||lib||community||cafe)) return null;
   return {id:`osm-${e.type}-${e.id}`,title:t.name,kind:cowork?'private':'public',universe:'computer',city:t['addr:city']||t['addr:town']||t['addr:village']||'Autour de Rennes',lat,lng,distance:+km(ORIGIN.lat,ORIGIN.lng,lat,lng).toFixed(1),rating:0,reviews:0,price:0,capacity:12,quiet:lib||cowork,wifi:t.internet_access==='wlan'||t.wifi==='yes',tools:false,live:false,updatedMins:1440,host:'OpenStreetMap',desc:cowork?'Espace de coworking référencé dans OpenStreetMap.':lib?'Bibliothèque ou médiathèque publique.':community?'Lieu collectif ou associatif susceptible d’accueillir du travail nomade.':'Café à vérifier pour le travail sur ordinateur.',rules:'Vérifier horaires, Wi‑Fi, prises et conditions d’accès avant déplacement.',amenities:[cowork?'Coworking':lib?'Bibliothèque':community?'Tiers-lieu / collectif':'Café',t.internet_access==='wlan'?'Wi‑Fi':'Wi‑Fi à confirmer'],slots:[t.opening_hours||'Horaires à vérifier'],address:[t['addr:housenumber'],t['addr:street']].filter(Boolean).join(' '),phone:t.phone||t['contact:phone']||'',website:t.website||t['contact:website']||'',openingHours:t.opening_hours||'',placeType:cowork?'coworking':lib?'library':community?'third_place':'cafe',verifiedAt:today,mapAccuracy:'OpenStreetMap'};
 }).filter(Boolean);
}catch(e){console.warn('Overpass unavailable:',e.message)}

const seen=new Set();
const all=[...curated,...osm].filter(p=>{p.distance=p.distance??+km(ORIGIN.lat,ORIGIN.lng,p.lat,p.lng).toFixed(1);p.rating=p.rating??0;p.reviews=p.reviews??0;p.capacity=p.capacity??12;p.live=false;p.updatedMins=1440;p.host=p.source||p.host||'Source vérifiée';p.rules=p.rules||'Vérifier les conditions avant déplacement.';p.slots=p.slots||['Horaires à vérifier'];p.verifiedAt=p.verifiedAt||today;const k=(p.title+'|'+p.city).toLowerCase();if(seen.has(k))return false;seen.add(k);return p.distance<=55;});
...
const js=`(()=>{try{const add=${JSON.stringify(all)};const key='coworkit3_places';const cur=JSON.parse(localStorage.getItem(key)||'[]');const by=new Map(cur.map(p=>[p.id,p]));for(const p of add)if(!by.has(p.id))by.set(p.id,p);localStorage.setItem(key,JSON.stringify([...by.values()]));}catch(e){console.warn('Cowork catalog',e)}})();\n`;
