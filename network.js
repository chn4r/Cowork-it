...
function readConfig(){try{return JSON.parse(localStorage.getItem(KEY)||'{"mode":"local"}')}catch{return {mode:'local'}}}
function setConfig(c){config={...config,...c};localStorage.setItem(KEY,JSON.stringify({...config,token:config.mode==='api'?'':config.token||''}))}
...
function supabaseHeaders(anonKey,extra={}){const bearer=accessToken()||anonKey;return {'apikey':anonKey,'Authorization':'Bearer '+bearer,'Content-Type':'application/json','Prefer':'resolution=merge-duplicates,return=minimal',...extra}}
...
async function flush(){
 if((config.mode||'local')==='local')return {sent:0,pending:0};
 let q=getQueue(),sent=0,remain=[];
 for(let i=0;i<q.length;i++){
   const item=q[i];
   try{await send(item);sent++}catch(e){remain.push(item);if(e.name==='AbortError'||/Failed to fetch|NetworkError/i.test(e.message)){remain.push(...q.slice(i+1));break}}
 }
 setQueue(remain);return {sent,pending:remain.length};
}
async function send(item){if(config.mode==='supabase')return sendSupabase(item);if(config.mode==='api')return sendApi(item);throw new Error('Mode réseau inconnu')}
async function sendApi(item){
 const url=(config.apiUrl||'').replace(/\/$/,'');if(!url)throw new Error('URL API manquante');
 const token=sessionStorage.getItem('coworkit3_api_token')||config.token||'';
 const r=await fetchTimeout(url+'/sync',{method:'POST',headers:{'Content-Type':'application/json',...(token?{'Authorization':'Bearer '+token}:{})},body:JSON.stringify(item)});
 if(!r.ok)throw new Error('HTTP '+r.status);return true;
}
function placeRow(p,userId){return {id:String(p.id),owner_id:userId,title:p.title,kind:p.kind,universe:p.universe,city:p.city,latitude:p.lat,longitude:p.lng,distance_km:p.distance,rating:p.rating,reviews:p.reviews,price_per_day:p.price,capacity:p.capacity,quiet:p.quiet,wifi:p.wifi,tools:p.tools,live:p.live,availability_updated_at:new Date(Date.now()-(p.updatedMins||0)*60000).toISOString(),host_name:p.host,description:p.desc,rules:p.rules,amenities:p.amenities,slots:p.slots,payload:p}}
async function upsert(url,token,table,rows,onConflict='id'){
 if(!rows?.length)return;
 const r=await fetchTimeout(`${url}/rest/v1/${table}?on_conflict=${encodeURIComponent(onConflict)}`,{method:'POST',headers:supabaseHeaders(token),body:JSON.stringify(rows)});
 if(!r.ok){const t=await r.text().catch(()=>String(r.status));throw new Error(`Supabase ${table}: ${r.status} ${t.slice(0,160)}`)}
}
async function sendSupabase(item){
 ...
 const user=currentUser();if(!user?.id||!accessToken())throw new Error('Connexion utilisateur requise pour synchroniser');
 if(item.type==='snapshot'){
   const owned=(item.payload.places||[]).filter(p=>p.localOwned===true||p.host==='Vous').map(p=>placeRow(p,user.id));
   await upsert(url,token,'places',owned);
   const favoriteRows=(item.payload.favorites||[]).map(id=>({user_id:user.id,place_id:String(id)}));
   if(favoriteRows.length)await upsert(url,token,'favorites',favoriteRows,'user_id,place_id');
   const threadRows=(item.payload.threads||[]).map(t=>({id:String(t.id),owner_id:user.id,person_name:t.person,initials:t.initials||'',context:t.context||{},payload:t}));
   if(threadRows.length)await upsert(url,token,'threads',threadRows);
 }
 return true;
}
...
async function signOut(){
 const url=(config.supabaseUrl||'').replace(/\/$/,'');const key=config.anonKey||'',jwt=accessToken();
 if(url&&key&&jwt){try{await fetchTimeout(url+'/auth/v1/logout',{method:'POST',headers:{'apikey':key,'Authorization':'Bearer '+jwt}},5000)}catch{}}
 try{sessionStorage.removeItem('coworkit3_access_token');sessionStorage.removeItem('coworkit3_user')}catch{}
}
